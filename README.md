...
### 구현 기능 목록
- [x] 1~45의 숫자 범위에서 중복되지 않는 6개의 숫자를 뽑아 1개의 로또 발행
- [x] Lotto 클래스의 validate 메서드에 유효성 검사 추가: 로또 번호의 숫자 범위, 중복
- [x] 로또 번호는 오름차순으로 정렬하여 출력
- [x] 입력 받은 로또 구입 금액의 유효성 검사
- [x] 구입 금액에 해당하는 만큼 로또 발행 후 발행한 로또 수량 및 번호 출력
- [x] 쉼표(,)를 기준으로 구분해 입력 받은 당첨 번호의 유효성 검사
- [x] 입력 받은 보너스 번호의 유효성 검사: 입력 받은 당첨 번호와의 중복
- [x] 등수에 따른 당첨 기준과 금액 관리
- [x] 사용자가 구매한 로또 번호와 당첨 번호를 비교하여 당첨 내역 집계 후 출력
- [x] 수익률 계산 후 소수점 둘째 자리에서 반올림해 출력
- [x] 전체 흐름 제어